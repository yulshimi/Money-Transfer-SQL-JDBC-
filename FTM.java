...
public class FTM
{
	...
	public static void main(String[] args) throws SQLException, ClassNotFoundException
	{
		...
		stmt_1.executeUpdate(updateStr_1);
		ResultSet rset_1 = stmt_1.executeQuery("SELECT*FROM middle_table");
		ResultSet rset_T_old;
		String delete_temp_T = "DROP TABLE temp_T";
		String create_temp_T = "SELECT*INTO temp_T FROM result_table";
		String combined_view = "CREATE VIEW combinedView AS SELECT X.src_name, Y.dest_name, Y.timestamp from result_table X, result_table Y WHERE X.dest_name = Y.src_name";
		PreparedStatement updateStud = null;
		while(rset_1.next())
		{
			rset_T_old = stmt_1.executeQuery("SELECT DISTINCT *FROM result_table");
			stmt_1.executeUpdate(combined_view);
			String updateStr_T = "INSERT INTO result_table SELECT*FROM combinedView";
			stmt_1.executeUpdate(updateStr_T);
			stmt_1.executeUpdate("DROP VIEW combinedView");
			stmt_1.executeUpdate(create_temp_T);
			...
			rset_1.close();
			rset_1 = stmt_1.executeQuery("SELECT*FROM temp_T");
			stmt_1.executeUpdate(delete_temp_T);
			rset_T_old.close();
		}
